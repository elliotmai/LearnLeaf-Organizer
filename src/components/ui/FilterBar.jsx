        ...
        <div style={{ padding: "0 16px 16px", display: "flex", gap: "12px", flexWrap: "wrap", borderTop: "1px solid #f0f4f2", paddingTop: "12px" }}>
          {subjects.length > 0 && (
            <select className="ll-input ll-select" style={{ width: "auto", minWidth: "140px" }}
              ...
              {[...subjects].sort((a, b) => (a.subjectName || "").localeCompare(b.subjectName || "", undefined, { sensitivity: "base" })).map(s => <option key={s.subjectId} value={s.subjectId}>{s.subjectName}</option>)}
            </select>
          )}
          {showProject && projects.length > 0 && (
            <select className="ll-input ll-select" style={{ width: "auto", minWidth: "140px" }}
              ...
              {[...projects].sort((a, b) => (a.projectName || "").localeCompare(b.projectName || "", undefined, { sensitivity: "base" })).map(p => <option key={p.projectId} value={p.projectId}>{p.projectName}</option>)}
            </select>
          )}
