...
export function formatDateDisplay(input, dateFormat) {
  ...
  if (dateFormat === 'YYYY-MM-DD') return date.toLocaleDateString('en-CA');
  if (dateFormat === 'DD/MM/YYYY') return date.toLocaleDateString('en-GB');
...
/**
 * Full refresh — re-pulls everything from Firebase.
 * Used by pull-to-refresh on mobile and by the auth-restore path on web page reload.
 *
 * If `clear` is true, IDB stores are wiped first so records deleted in Firebase
 * since the last sync don't linger locally. Pages re-render off a `dataVersion`
 * bump after this resolves, so the brief empty window isn't user-visible.
 */
export async function refreshAllData(uid, { clear = false } = {}) {
  ...
  await fetchCriticalData(uid);
  await backgroundFetchRemaining(uid);
}
