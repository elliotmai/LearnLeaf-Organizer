...
export default function ProjectTasksPage() {
  ...
  const load = useCallback(async () => {
    ...
    const active = allTasks.filter(t => t.taskStatus!=="Completed" && t.taskProject===projectId);
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",minHeight:"100vh" }}>
      ...
      <main style={{ flex:1,maxWidth:"1280px",margin:"0 auto",width:"100%",padding:"24px 24px 80px",boxSizing:"border-box" }}>
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"20px",flexWrap:"wrap",gap:"12px" }}>
          <div>
            <h1 style={{ fontFamily:"Playfair Display,serif",fontSize:"1.6rem",fontWeight:700,color:"#907474",margin:"0 0 2px" }}>
              {project ? project.projectName : "Project Tasks"}
            </h1>
            <p style={{ color:"#9ca3af",fontSize:"0.85rem",margin:0 }}>{filtered.length} active task{filtered.length!==1?"s":""}</p>
          </div>
        ...
        <FilterBar filterCriteria={filter} onChange={u => setFilter(f => ({...f,...u}))} onClear={() => setFilter({ ...EMPTY_FILTER })} subjects={subjects} projects={projects} showProject={false} />
      ...
      <TaskForm open={sidebarOpen} onClose={() => setSidebarOpen(false)} task={editingTask} subjects={subjects} projects={projects} onSave={handleSave} defaultProjectId={projectId} />
