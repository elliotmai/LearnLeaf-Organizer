...
import { deleteTask, editTask, deleteSubject, deleteProject, reactivateSubject, reactivateProject, sortTasks, sortSubjects, sortProjects, formatDateDisplay } from "../LearnLeaf_Functions.jsx";
...
export default function ArchivePage() {
  ...
  const load = useCallback(async () => {
    ...
    const [taskSnap, subjectSnap, projectSnap, allSubjectSnap, allProjectSnap] = await Promise.all([
    ...
    const subjects  = subjectSnap.docs.map(d => ({ subjectId: d.id, ...d.data() }));
    const projects  = projectSnap.docs.map(d => ({ projectId: d.id, ...d.data() }));
    const allSubs   = allSubjectSnap.docs.map(d => ({ subjectId: d.id, ...d.data() }));
    const allProjs  = allProjectSnap.docs.map(d => ({ projectId: d.id, ...d.data() }));
    const completed = taskSnap.docs.map(d => {
      const data = d.data();
      return {
        taskId: d.id, ...data,
        taskSubject: allSubs.find(s => s.subjectId === data.taskSubject?.id) || null,
        taskProject: allProjs.find(p => p.projectId === data.taskProject?.id) || null,
      };
    });

    setCompletedTasks(sortTasks(completed));
    setArchivedSubjects(sortSubjects(subjects));
    setArchivedProjects(sortProjects(projects));
    setAllSubjects(allSubs.filter(s => s.subjectStatus === "Active"));
    setAllProjects(allProjs.filter(p => p.projectStatus === "Active"));
    setLoading(false);
  }, []);
  ...
  const handleReactivateTask = async (task) => {
    ...
    await load();
    setToast({ message:"Task reactivated — set to In Progress!", type:"success" });
  };

  const handleEditTask = (task) => {
    // Flatten subject/project refs for the form
    setEditingTask({
      ...task,
  ...
  const actionBtn = (label, onClick, color = "#6b7280", bg = "#f3f4f6", border = "none") => (
    <button onClick={onClick}
      style={{ padding:"4px 8px",borderRadius:"6px",border,background:bg,color,cursor:"pointer",fontSize:"0.75rem",fontWeight:500,whiteSpace:"nowrap" }}>
      {label}
    </button>
  );
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",minHeight:"100vh" }}>
                ...
                <div className="ll-card" style={{ marginTop:"8px",overflow:"hidden" }}>
                  <div style={{ overflowX:"auto" }}>
                    <table style={{ width:"100%",borderCollapse:"collapse" }}>
                      ...
                      <tbody>
                        {filteredTasks.map(t => (
                          <tr key={t.taskId} style={{ transition:"background 150ms" }}
                            ...
                            <td style={cell}>
                              <span style={{ fontSize:"0.7rem",padding:"2px 7px",borderRadius:"999px",fontWeight:600,
                                background:t.taskPriority==="High"?"rgba(243,22,30,0.1)":t.taskPriority==="Medium"?"rgba(159,108,91,0.1)":"rgba(182,205,200,0.3)",
                                color:t.taskPriority==="High"?"#c01018":t.taskPriority==="Medium"?"#7d5244":"#355147" }}>
                                {t.taskPriority}
                              </span>
                            </td>
                            ...
                            <td style={cell}>
                              <div style={{ display:"flex",gap:"5px",flexWrap:"wrap" }}>
                                {actionBtn("Reactivate", () => handleReactivateTask(t), "#355147", "rgba(53,81,71,0.08)", "1px solid rgba(53,81,71,0.2)")}
                                {actionBtn("Edit", () => handleEditTask(t), "#5B8E9F", "rgba(91,142,159,0.08)", "1px solid rgba(91,142,159,0.2)")}
                                {actionBtn("Delete", () => setDeleteInfo({ type:"task",id:t.taskId }), "#F3161E", "rgba(243,22,30,0.08)")}
                              </div>
                            </td>
                          </tr>
                        ))}
                    ...
                    <div key={s.subjectId} className="ll-card" style={{ padding:"14px",borderTop:`3px solid ${s.subjectColor||"#B6CDC8"}` }}>
                      ...
                      <div style={{ display:"flex",gap:"6px",flexWrap:"wrap" }}>
                        ...
                        <button onClick={() => handleEditSubject(s)}
                          style={{ padding:"5px 8px",borderRadius:"7px",border:"1px solid rgba(91,142,159,0.3)",background:"rgba(91,142,159,0.08)",color:"#5B8E9F",cursor:"pointer",fontSize:"0.75rem",fontWeight:500 }}>
                          Edit
                        </button>
                  ...
                  {filteredProjects.map(p => (
                    <div key={p.projectId} className="ll-card" style={{ padding:"14px" }}>
                      ...
                      {p.projectDescription && (
                        <p style={{ margin:"0 0 10px",fontSize:"0.78rem",color:"#6b7280",lineHeight:1.4,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden" }}>
                          {p.projectDescription}
                        </p>
                      )}
                      <div style={{ display:"flex",gap:"6px",flexWrap:"wrap" }}>
      ...
      <TaskForm
        ...
        onSave={async () => { await load(); setToast({ message:"Task updated!", type:"success" }); }} />

      <SubjectForm
        open={subjectFormOpen}
        onClose={() => setSubjectFormOpen(false)}
        subject={editingSubject}
        onSave={async () => { await load(); setToast({ message:"Subject updated!", type:"success" }); }} />
      ...
      <ProjectForm
        ...
        subjects={allSubjects}
        onSave={async () => { await load(); setToast({ message:"Project updated!", type:"success" }); }} />
